...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
leicester_2011OAC <- readr::read_csv(paste0(Sys.getenv("GRANOLARR_HOME"), "/data/", "2011_OAC_supgrp_Leicester.csv"))
leicester_2011OAC %>% 
  print(n= 3, width = 60)
```
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
leicester_2011OAC <- readr::read_csv(
    paste0(Sys.getenv("GRANOLARR_HOME"), "/data/", "2011_OAC_supgrp_Leicester.csv"),
    col_types = cols(
      OA11CD  = col_character(),
      LSOA11CD  = col_character(),
      supgrpcode  = col_character(),
...
readr::read_csv(
  ...
  dplyr::filter(supgrpcode == "6") %>%
  readr::write_csv(
    paste0(Sys.getenv("GRANOLARR_HOME"), "/data/", "2011_OAC_supgrp_Leicester_supgrp6.csv")
  )
```
...
readr::read_csv(
  ...
  dplyr::filter(supgrpcode == "6") %>%
  readr::write_tsv(
    paste0(Sys.getenv("GRANOLARR_HOME"), "/data/", "2011_OAC_supgrp_Leicester_supgrp6.tsv")
  )
```
