...
penguins_bm_bd_dbscan2 <- penguins_norm %>%
  add_column(bm_bd_dbscan = penguins_dbscan_result2 %$% cluster)
```
...
penguins_bm_bd_dbscan3 <- penguins_norm %>%
  add_column(bm_bd_dbscan = penguins_dbscan_result3 %$% cluster)
```
...
penguins_bm_bd_dbscan3 %>%
  ggplot(aes(
    ...
    colour = factor(bm_bd_dbscan))) +
  geom_point() +
  scale_color_manual(values = c("#969696", "#a6cee3", "#1f78b4", "#b2df8a", "#33a02c", "#fb9a99", "#e31a1c", "#fdbf6f", "#ff7f00", "#cab2d6", "#6a3d9a", "#ffff99", "#b15928")) +
  ggplot2::theme_bw() +
  theme(legend.position = "bottom")
```
...
penguins_norm_bills <- 
  penguins_norm_bills %>%
  add_column(bills_hclust = cutree(penguins_norm_bills_hclust, k = 3))
...
penguins_norm_bills <- 
  penguins_norm_bills %>%
  add_column(bills_bclust = clusters.bclust(penguins_norm_bills_bclust, 3))
...
penguins_norm_bills <- 
  penguins_norm_bills %>%
  add_column(bills_dbscan = penguins_norm_bills_dbscan %$% cluster)
```
