...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
# Load Indexes of Multiple deprivation data
leicester_IMD2015 <- read_csv(paste0(Sys.getenv("GRANOLARR_HOME"), "/data/", "IndexesMultipleDeprivation2015_Leicester.csv"))
```
...
read_csv(paste0(Sys.getenv("GRANOLARR_HOME"), "/data/", "2011_OAC_Raw_uVariables_Leicester.csv")) %>%
  select(OA11CD, supgrpcode, supgrpname) %>%
  filter(supgrpcode == 6) %>%
  write_csv("~/Leicester_Suburbanites.csv")
```
