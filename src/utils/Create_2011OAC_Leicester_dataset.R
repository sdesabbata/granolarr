...
ifelse(!dir.exists(file.path(Sys.getenv("GRANOLARR_HOME"), paste0(Sys.getenv("GRANOLARR_HOME"), "/tmp"))), dir.create(file.path(Sys.getenv("GRANOLARR_HOME"), paste0(Sys.getenv("GRANOLARR_HOME"), "/tmp"))), FALSE)

# Extract data files
if (!dir.exists(file.path(paste0(Sys.getenv("GRANOLARR_HOME"), "/tmp"), "2011 OAC 167 Variables"))) {
  unzip(paste0(Sys.getenv("GRANOLARR_HOME"), "/Storage/2011 OAC Variables - 167 Initial.zip"), exdir = paste0(Sys.getenv("GRANOLARR_HOME"), "/tmp"))
}
...
vars_2011OAC <- read_csv(paste0(Sys.getenv("GRANOLARR_HOME"), "/tmp/2011 OAC 167 Variables/2011_OAC_Raw_uVariables.csv"))

areas_lookup <- read_csv(paste0(Sys.getenv("GRANOLARR_HOME"), "/tmp/OA11_LSOA11_MSOA11_LAD11_EW_LU/OA11_LSOA11_MSOA11_LAD11_EW_LUv2.csv"),
    col_types = cols(
